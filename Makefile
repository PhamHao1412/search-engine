...
PRODUCT_MIGRATIONS_DIR=backend/product-service/scripts/migrations
SEARCH_MIGRATIONS_DIR=backend/search-service/scripts/migrations
...
help:
	...
	@echo "  make migrate-product-up    - Áp dụng migrations lên schema 'product' (dùng backend/product-service/.env)"
	@echo "  make migrate-search-up     - Áp dụng migrations lên schema 'search' (dùng backend/search-service/.env)"
...
migrate-product-up:
	@export $$(cat backend/product-service/.env | grep -v '^#' | xargs) && \
...
migrate-search-up:
	@export $$(cat backend/search-service/.env | grep -v '^#' | xargs) && \
	psql "postgres://$$DB_USER:$$DB_PASSWORD@$$DB_HOST:$$DB_PORT/$$DB_NAME?sslmode=$$DB_SSLMODE" -c "CREATE SCHEMA IF NOT EXISTS $$DB_SCHEMA;" && \
...
run-product:
	cd backend/product-service && go run cmd/serverd/main.go

run-search-api:
	cd backend/search-service && go run cmd/serverd/main.go

run-search-worker:
	cd backend/search-service && go run cmd/workerd/main.go

# Tests
test-product:
	cd backend/product-service && env GOCACHE=.cache/go-build GOPATH=.go go test -v ./...

test-search:
	cd backend/search-service && env GOCACHE=.cache/go-build GOPATH=.go go test -v ./...

test: test-product test-search

clean:
	rm -rf backend/product-service/.tmp backend/product-service/.cache backend/product-service/.go
	rm -rf backend/search-service/.tmp backend/search-service/.cache backend/search-service/.go
